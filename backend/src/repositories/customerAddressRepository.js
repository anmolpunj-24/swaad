...
const getAllCustomerAddressesRepo = async (customerUuid) => {
  return await customerAddressModel
    .find({ customerUuid })
    .select(
      "_id addressLine_1 addressLine_2 country state postalCode city addressType isDefault createdAt",
    )
    .sort("-createdAt")
    .lean();
};
...
const getOneCustomerAddressRepo = async (customerUuid, addressId) => {
  return await customerAddressModel
    ...
    .select(
      "_id addressLine_1 addressLine_2 country state postalCode addressType city isDefault createdAt",
    )
    .lean();
};
